...
[M,notearray]=detectMono(left,m,n,Fs,w,k,0.37);

M=removeInfFFT(M);

M=equalizeNotes(M,n,Fs,3);
...
%}
%%
%monoSynthAdd(M,Fs,0.3,0.6,'sin','sawtooth','square',0.7,1,0.1,0.8,0.1,0.1);
%monoSynthFM(M,Fs,1,5,'sin','sin',0.7,1,0.1,0.8,0.1,0.1);
%monoClarinet(M,Fs,0.1,1,0.3,0.8,0.4,0.2);
%%
midi_new = matrix2midi(M);
rawmid=writemidi(midi_new, 'testout.mid');
%soundsc(midi2audio('testout.mid',Fs,'fm'),Fs);
%%
ispmidi = isp_midiread('testout.mid');
soundfont=fullfile(isp_toolboxpath, 'Saber_5ths_and_3rds.sf2');
